...
namespace davidglitch04\EpicCustomAlerts\updater;
...
class Utils{
    ...
    protected static function downloadUpdate(string $url) : void {
        ...
        $path = $plugin->getDataFolder()."tmp/EpicCustomAlerts.phar";
	...
	public static function handleDownload(string $path, int $status) : void {
        ...
        @rename($path, Server::getInstance()->getPluginPath()."/EpicCustomAlerts.phar");
        ...
        @rename(Server::getInstance()->getPluginPath()."/".Utils::getFileName(), Server::getInstance()->getPluginPath()."/EpicCustomAlerts.phar.old"); //failsafe i guess.
        Server::getInstance()->getLogger()->warning("Update complete, restart your server to load the new updated version.");
        return;
    }
	/**
